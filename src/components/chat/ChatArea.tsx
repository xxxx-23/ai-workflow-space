...
export const ChatArea: React.FC = () => {
  ...
  const handleSend = async () => {
    if ((!input.trim() && attachments.length === 0) || !activeId || isGenerating) return;
                      ...
                      {msg.attachments && msg.attachments.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-2">
                           {msg.attachments.map(att => (
                             att.type === 'image' ? (
                               <img key={att.id} src={att.data} alt="attachment" className="max-w-xs max-h-60 object-contain rounded-lg border border-black/10 dark:border-white/10" />
                             ) : (
                               <div key={att.id} className="p-2 bg-black/10 dark:bg-white/10 rounded text-sm text-inherit flex items-center gap-1">📎 <span>{att.name}</span> <span className="text-[10px] opacity-70">{(att.size ? att.size / 1024 : 0).toFixed(0)}KB</span></div>
                             )
                           ))}
                        </div>
                      )}
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-2 relative shadow-lg rounded-xl bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 p-2 text-gray-900 dark:text-gray-100 pointer-events-auto">
          ...
          {attachments.length > 0 && (
            <div className="flex gap-2 flex-wrap px-2 pt-2">
              {attachments.map(att => (
                <div key={att.id} className="relative group w-16 h-16 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
                  {att.type === 'image' ? (
                    <img src={att.data} alt={att.name} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-[10px] items-center flex text-center break-all p-1 leading-tight">{att.name.slice(0,10)}...</span>
                  )}
                  <button onClick={() => removeAttachment(att.id)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">✕</button>
                </div>
              ))}
            </div>
          )}
